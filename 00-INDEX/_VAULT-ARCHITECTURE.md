...
│   │   ├── geo-奥尔德林学院.md  # 漂浮岛群学院（区域主档）
│   │   └── academy/             # 学院子地点目录（建筑/功能级）
│   ├── history/               # 历史子域（待建设）
│   │   └── _index.md
│   ├── culture/               # 文化子域（待建设）
│   │   └── _index.md
...
| 文件 | 职责 |
|------|------|
| `char-*.md` | 角色当前主档。记录已经确认、可以归入当前正史的角色信息，并随剧情推进持续完善 |
| `arc-*.md` | 剧情组织层。同一文件内区分 `已确认内容`、`参考设计`、`待定事项` |
| `relationships-女主群像.md` | 女主之间长期成立的结构性调性关系，不替代具体剧情事件 |

补充规则：

1. **已确认且已揭露**的内容，可以进入 `char`
2. **已确认但未揭露**的真相，仍留在 `plot/arc`，不提前写回 `char`
3. **未定设计 / 候选走向**，统一放在 `plot/arc` 的 `参考设计` 中
